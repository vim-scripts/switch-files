...
" Switch between similar files (only differing in path/ file endings) easily
...
" start region 'file <dotvim>/autoload/vl/lib/files/switch_files.vim'
" "|fld   description : <+ really short description to get a picture. less than 2 sentences if possible +>
" "|fld   keywords : <+ this is used to index / group scripts ?+> 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Feb 14 18:52:02
" "|fld   version: 0.1
" "|fld   dependencies: <+ vim-python, vim-perl, unix-tool sed, ... +>
" "|fld   contributors : <+credits to+>
...
" "|
" "|p     If you need some custom function drop me a mail and I'll tell you how
" "|+     to add it
...
" 
" " 
" function! vl#lib#files#switch_files#FileAlternativesByRegex(pat, subst, file)
"   let newF = substitute(a:file, a:pat, a:subst)
" endfunction
" 
" function! vl#lib#files#switch_files#SwitchFile()
"   let list = vl#lib#vimscript#scriptsettings#SettingList(
"         \ 'switchfiles_create_opportunities_list')
"   let files = vl#lib#listdict#list#Concat(
"         \ vl#lib#listdict#list#MapCopy(list, 
"         \ ' vl#lib#vimscript#closure#Call(Val,['.string(expand('%:p')).'])' ) 
"         \ )
"   if len(files)==0
"     echo "no files found to switch to"
"     return 
"   endif
"   let files = vl#lib#listdict#list#Unique(files)
"   call filter(files, 'v:val != '.string(expand('%:p')))
" 
"   let file = vl#ui#userSelection#LetUserSelectIfThereIsAChoiceAppendExtraInfo(
"         \ 'select file to switch to', files, 
"         \ 'filereadable(Val) ? "" : "new"')
"   " try preserving line
"   silent exec 'e '.file
"   silent! exec 'normal /'.current."\<cr>"
" endfunction
" 
" function! vl#lib#files#switch_files#SwitchFileAdd(item)
"   call vl#lib#vimscript#scriptsettings#GetOrDefine(
"         \ 'b:switchfiles_create_opportunities_list', string([]))
"   call add(b:switchfiles_create_opportunities_list, a:item)
" endfunction
" 
" "used by vl#lib#files#switch_files#SwitchFileRegisterLocalExec
" function! vl#lib#files#switch_files#SwitchFileExec(expr, file)
"   let Val = a:file
"   exec 'return '.a:expr
" endfunction
" 
" function! vl#lib#files#switch_files#SwitchFileRegisterLocalExec(toExecute)
"   call vl#lib#files#switch_files#SwitchFileAdd(
"         \ { 'func' : 'vl#lib#files#switch_files#SwitchFileExec' 
"         \ , 'args': [a:toExecute] } )
" endfunction
" 
" function! vl#lib#files#switch_files#SwitchFileRegisterLocalRegex(pat,subst)
"   let pats = vl#lib#listdict#list#ToList(a:pat)
"   let substs = vl#lib#listdict#list#ToList(a:subst)
"   for p in pats
"     for s in substs
"       call vl#lib#files#switch_files#SwitchFileRegisterLocalExec(
"             \ 'substitute(Val,'.string(p).','.string(s).',"")')
"     endfor
"   endfor
" endfunction
" function! vl#lib#files#switch_files#SwitchFileRegisterLocalRegexSwap(pat,subst)
"   call vl#lib#files#switch_files#SwitchFileRegisterLocalRegex(a:pat, a:subst)
"   call vl#lib#files#switch_files#SwitchFileRegisterLocalRegex(a:subst, a:pat)
" endfunction
" end region 'file <dotvim>/autoload/vl/lib/files/switch_files.vim'
...
" start region 'file <dotvim>/autoload/vl/lib/vimscript/closure.vim'
" "|fld   description : some object boxed closure array (pass not only a
" "function but a function with configuration
" "|fld   keywords : closure vimscript 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Jul 07 17:15:13
" "|fld   version: 0.0
" "|
" "|H1__  Documentation
" "|
" "|p     Having quite a lot of functional background now I think closures are a
" "|      really nice thing. This is a hack to get some of the benefits in vim
" "|      script as well
" 
" 
" "|func  Call either 
" "|       a) a string (interpreted as function)
" "|       b) a func ref
" "|       c) some kind of closure or partially applied function
" "|     See also :h Dictionary-function
" "|     Examples (They do all the same):
" "|     call ..#Call('string',2)
" "|     call ..#Call(function('string'),2)
" "|     call ..#Call({func : 'string', args: [2])
" function! vl#lib#vimscript#closure#Call(obj, ...)
"   exec vl#lib#brief#args#GetOptionalArg('args',string([]))
"   if type(a:obj) == 1
"     return  call(function(a:obj), args)
"   elseif type(a:obj) == 2
"     return  call(a:obj, args)
"   else " assume it's a closure
"     let d = a:obj
"     if type(d.func) == 1
"       let F = function(d.func)
"     else
"       let F = d.func
"     endif
"     if exists('d.args')
"       return  call(F, d.args + args, d)
"     else
"       return  call(F, args, d)
"     endif
"   endif
" endfunction
" end region 'file <dotvim>/autoload/vl/lib/vimscript/closure.vim'
